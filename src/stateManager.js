...
class StateManager extends EventEmitter {
  ...
  parseModifiedStreams(oldCode, newCode) {
    ...
    for (let i = 1; i <= 8; i++) {
      const streamKey = `d${i}`;
      const oldStream = oldStreams[streamKey] || '';
      const newStream = newStreams[streamKey] || '';
  
      if (oldStream !== newStream) {
        modified.push(i - 1); // Convert to zero-based index for LED columns
      }
    }
  ...
  clearModifiedStreams(row, streamIndex = null) {
    // Ensure that modifiedStreams for this row exists
    ...
    if (streamIndex !== null) {
      ...
      delete this.modifiedStreams[row]; // Remove all modifications for the row
    }
  }
